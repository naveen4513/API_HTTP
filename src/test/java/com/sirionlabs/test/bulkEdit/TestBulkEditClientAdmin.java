...
public class TestBulkEditClientAdmin extends TestAPIBase {

	private final static Logger logger = LoggerFactory.getLogger(TestBulkEditClientAdmin.class);
	...
	/*
	Verify that Bulk Email can be configured from Client Admin and Created mail is displayed in email name drop down.
	 */
	@Test(enabled = false)
	public void testBulkEditEmailConfiguration() {
	...
	/*
	Verify Bulk Edit Check Permission is present in all related entities permission page.
	Bulk Edit Checkbox is selectable at User Role Group.
	 */
	@Test
	public void testBulkEditPermissionInUserRoleGroup() {
		CustomAssert csAssert = new CustomAssert();

		try {
			logger.info("Starting Test TC-C9099: Verify Bulk Edit Permission.");
			...
			for (String entity : entitiesToTest) {
				...
				csAssert.assertTrue(bulkEditFound, "Bulk Edit CheckBox not found in User Role Groups Update API Response for Entity " + entity);
			}
		} catch (SkipException e) {
			throw new SkipException(e.getMessage());
		} catch (Exception e) {
			csAssert.assertTrue(false, "Exception while Validating Bulk Edit Permission TC-C9099. " + e.getMessage());
		}
		csAssert.assertAll();
	}
	...
	/*
	 Verify Bulk Edit Checkbox is present or not in Supplier Access.
	 */
	@Test
	public void testBulkEditPermissionInSupplierAccess() {
		CustomAssert csAssert = new CustomAssert();

		try {
			logger.info("Starting Test TC-C9109: Verify Bulk Edit Checkbox in Supplier Access.");
			...
			for (String entity : entitiesToTest) {
				logger.info("Validating Bulk Edit Permission on Supplier Access Page for Entity {}", entity);
				boolean bulkEditFound = false;
				...
				csAssert.assertTrue(bulkEditFound, "Bulk Edit CheckBox not found in Supplier Access Update API Response for Entity " + entity);
			}
		} catch (SkipException e) {
			throw new SkipException(e.getMessage());
		} catch (Exception e) {
			csAssert.assertTrue(false, "Exception while Validating TC-C9109 Bulk Edit Checkbox in Supplier Access. " + e.getMessage());
		}
		csAssert.assertAll();
	}
	...
	/*
	 Verify Bulk Edit Checkbox is selectable at Supplier Access at Vendor Hierarchy Level.
	 */
	@Test
	public void testBulkEditPermissionInSupplierAccessAtVendorHierarchyLevel() {
		CustomAssert csAssert = new CustomAssert();

		try {
			logger.info("Starting Test TC-C9112: Verify Bulk Edit Checkbox in Supplier Access at VH Level.");
			...
			for (String entity : entitiesToTest) {
				...
				csAssert.assertTrue(bulkEditFound, "Bulk Edit CheckBox not found in Supplier Access at VH Level for Entity " + entity);
			}
		} catch (SkipException e) {
			throw new SkipException(e.getMessage());
		} catch (Exception e) {
			csAssert.assertTrue(false, "Exception while Validating TC-C9112 Bulk Edit Checkbox in Supplier Access at VH Level. " + e.getMessage());
		}
		csAssert.assertAll();
	}
}