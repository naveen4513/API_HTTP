...
public class TestFlowDownForAction {
    private final static Logger logger = LoggerFactory.getLogger(TestFlowDownForAction.class);
    ...
    public void TestRoleGroupFlowDownForContract() {
        ...
        parentEntityId = Integer.parseInt(ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "actionsflowdown", "contractid"));
        parentRoleGroupId = Integer.parseInt(ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "actionsflowdown", "contractrolegroupid"));
        ...
        if (message.contains("success")) {
            ...
            childRoleGroupId = RoleGroupDbHelper.getFlowDownRoleGroupId("Contract Manager","Actions");
        }

        // Create a new action from the selected parent contract
        String actionResponseString = Action.createAction("src/test/resources/Helper/EntityCreation/Action", "action.cfg", "src/test/resources/Helper/EntityCreation/Action", "actionExtraFields.cfg", "flowdown contract", true);
        childEntityId = CreateEntity.getNewEntityId(actionResponseString, "action");

        // Hit the show API for the newly created action and verify the role group value matches that of parent
        show = new Show();
        show.hitShow(18, childEntityId);
    ...
    public void TestRoleGroupFlowDownForSupplier() {
        ...
        String actionResponseString = Action.createAction("src/test/resources/Helper/EntityCreation/Action", "action.cfg", "src/test/resources/Helper/EntityCreation/Action", "actionExtraFields.cfg", "flowdown supplier", true);
        childEntityId = CreateEntity.getNewEntityId(actionResponseString, "action");
    ...
    public static String createEditPayload(String editPayload){
