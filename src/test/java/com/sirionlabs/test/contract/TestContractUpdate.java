...
public class TestContractUpdate {
	...
	private final static Logger logger = LoggerFactory.getLogger(TestContractUpdate.class);
	...
	public void beforeClass() throws ConfigurationException {
		configFilePath = ConfigureConstantFields.getConstantFieldsProperty("ContractUpdateTestConfigFilePath");
		configFileName = ConfigureConstantFields.getConstantFieldsProperty("ContractUpdateTestConfigFileName");
	...
	@DataProvider
	public Object[][] dataProviderForTestContractUpdate() throws ConfigurationException {
		logger.info("Setting all Contract Update Flows to Validate");
		...
		if (temp != null && !temp.trim().equalsIgnoreCase("false")) {
			...
			for (String flow : allFlows) {
				if (ParseConfigFile.containsSection(configFilePath, configFileName, flow.trim())) {
					...
					logger.info("Flow having name [{}] not found in Contract Update Config File.", flow.trim());
				}
			}
		}
	...
	@Test(dataProvider = "dataProviderForTestContractUpdate")
	public void testContractUpdate(String flowToTest) {
		CustomAssert csAssert = new CustomAssert();

		try {
			logger.info("Validating Contract Update Flow [{}]", flowToTest);
			...
			logger.info("Hitting Edit Get API for Contract Id {} and Flow [{}]", contractId, flowToTest);
			...
			if (ParseJsonResponse.validJsonResponse(editGetResponse)) {
				...
				if (fieldsPayloadMap.size() > 0) {
					...
					logger.info("Hitting Edit Post API for Contract Id {} and Flow [{}]", contractId, flowToTest);
					String editPostResponse = editObj.hitEdit("contracts", editPostPayload);
					...
					if (ParseJsonResponse.validJsonResponse(editPostResponse)) {
						...
						if (expectedResult.equalsIgnoreCase("success")) {
							if (!editStatus.equalsIgnoreCase("success")) {
								csAssert.assertTrue(false, "Couldn't update Contract Id " + contractId + " for Flow [" + flowToTest + "]");
							}
						} else {
							if (editStatus.equalsIgnoreCase("success")) {
								csAssert.assertTrue(false, "Contract Id " + contractId + " edited successfully for Flow [" + flowToTest +
										"] whereas it was expected to fail");
							}
						}

						//Restore Contract to Original State
						if (editStatus.equalsIgnoreCase("success")) {
							if (EntityOperationsHelper.restoreRecord("contracts", contractId, editGetResponse)) {
								logger.info("Contract Id {} restored Successfully.", contractId);
							} else {
								logger.error("Couldn't Restore Contract Id {}", contractId);
							}
						}
					} else {
						csAssert.assertTrue(false, "Edit Post API Response for Contract Id " + contractId + " and Flow [" + flowToTest +
								"] is an Invalid JSON.");
					}
				} else {
				...
				csAssert.assertTrue(false, "Edit Get API Response for Contract Id " + contractId + " and Flow [" + flowToTest + "] is an Invalid JSON.");
			}
		} catch (SkipException e) {
			throw new SkipException(e.getMessage());
		} catch (Exception e) {
			csAssert.assertTrue(false, "Exception while validating Contract Update for Flow [" + flowToTest + "]. " + e.getMessage());
		}
		csAssert.assertAll();
	}
}
