...
//SIR-10073
public class TestCurrencyRollUp_UpdRateCard {

    private final static Logger logger = LoggerFactory.getLogger(TestCurrencyRollUp_UpdRateCard.class);
    ...
    @Test(dependsOnMethods = "Test_ContractCreation_DC",priority = 90,enabled = true)
    public void Test_CurrencyRoll_UpdateRateCard() {
...
//            Updating rate card on contract
            updateNewRateCards(contractListMapWithLevel, newRateCardIdList2, newStartDateList2, newEndDateList2, customAssert);

            validateAggValuesDC(contractListMapWithLevel, contractListMap, childParentMap, "After updating new Rate Card And Direct Conversion ", customAssert);

            updateConversionTypeOnContracts(2, contractListMapWithLevel, customAssert);

//            Updating rate card on contract
            updateNewRateCards(contractListMapWithLevel, newRateCardIdList1, newStartDateList1, newEndDateList1, customAssert);
...
//                      Aggregate ACV = Sum Of ACV's (of all immediate child Entities)
    ...
    private void updateNewRateCards(HashMap<Integer, List<Integer>> contractListMapWithLevel,
                                    List<String> newRateCardIdList, List<String> newStartDateList, List<String> newEndDateList,
                                    CustomAssert customAssert) {
        ...
        try {
            ...
            for (Map.Entry<Integer, List<Integer>> entry : contractListMapWithLevel.entrySet()) {
                ...
                for (int i = 0; i < contractList.size(); i++) {
                    ...
                    updateSingleRateCard(contractId, newRateCardIdList.get(level -1), newStartDateList.get(level-1), newEndDateList.get(level-1), customAssert);

                }
            }

        } catch (Exception e) {
            logger.error("Exception while updating New Rate Cards On Contracts");
            customAssert.assertEquals("Exception while updating New Rate Cards On Contracts","Exception should not occur");
        }
