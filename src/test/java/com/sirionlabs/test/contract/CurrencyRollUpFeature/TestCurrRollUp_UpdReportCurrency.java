...
//SIR-10073
public class TestCurrRollUp_UpdReportCurrency {

    private final static Logger logger = LoggerFactory.getLogger(TestCurrRollUp_UpdReportCurrency.class);
    ...
    @Test(dependsOnMethods = "Test_ContractCreation_DC",priority = 80,enabled = true)
    public void Test_CurrencyRoll_Updation_ReportingCurrency() {
        ...
        try {
            ...
            //Updating contract currencies
            updateContractCurrencies(contractListMapWithLevel, customAssert);
            ...
            //Updating Indirect Conversion as conversion type
...
//                      Aggregate ACV = Sum Of ACV's (of all immediate child Entities)
    ...
    //    C3496
    private void updateContractCurrencies(HashMap<Integer, List<Integer>> contractListMapWithLevel, CustomAssert customAssert) {
        Edit edit = new Edit();
        String entityName = "contracts";
        ...
        try {
            ...
            for (Map.Entry<Integer, List<Integer>> entry : contractListMapWithLevel.entrySet()) {
                ...
                //Getting currency ID corresponding to levels
                int currencyId = Integer.parseInt(ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "update contract currencies", "level " + level));
                List<Integer> contractList = entry.getValue();
                ...
                for (int i = 0; i < contractList.size(); i++) {
                    ...
                    JSONArray values = editPayloadJson.getJSONObject("body").getJSONObject("data").getJSONObject("contractCurrencies").getJSONArray("values");
                    JSONObject newCurrencyValuesJson = new JSONObject();
                    newCurrencyValuesJson.put("id", currencyId);
                    values.put(values.length(), newCurrencyValuesJson);
                    editPayloadJson.getJSONObject("body").getJSONObject("data").getJSONObject("contractCurrencies").put("values", values);
                    editPayloadJson.getJSONObject("body").getJSONObject("data").getJSONObject("currency").getJSONObject("values").put("id", currencyId);
                    ...
                    if (!editResponse.contains("success")) {
                        customAssert.assertTrue(false, "Conversion type updated unsuccessfully on contracts ");
                    }
                }
            }

        } catch (Exception e) {
            logger.error("Exception while updating Conversion Tyeps On Contracts");
        }
