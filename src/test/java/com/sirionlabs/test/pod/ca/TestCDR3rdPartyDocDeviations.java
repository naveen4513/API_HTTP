...
public class TestCDR3rdPartyDocDeviations {
    private final static Logger logger = LoggerFactory.getLogger(TestCDR3rdPartyDocDeviations.class);
    ...
    @Test
    public void testC153913() {
        CustomAssert customAssert = new CustomAssert();
        int cdrId = -1;
        try{
            cdrId = createCDR(customAssert);
            HashMap<String, String> templateDetails = chooseTemplate(customAssert);
            if (addTheTemplateToCDR(cdrId, templateDetails, customAssert)){
                HashMap<String,Boolean> contractDocumentColumns = getContractDocumentColumns(cdrId, customAssert);
                customAssert.assertTrue(contractDocumentColumns.get("ColumnExist"),"Column Deviation Tracking Not Found. Hence, TC-C153913 failed.");
                customAssert.assertTrue(contractDocumentColumns.get("DeviationTracking"),"Deviation Tracking is Not Yes by default. Hence, TC-C153906 failed.");
            }else {
                logger.error("Template could not be added in the CDR.");
                customAssert.assertTrue(false,"Template could not be added in the CDR.");
            }
        }catch (Exception e){
            logger.error("Exception {} occurred while executing TC-C153913 and TC-C153906.",e.getMessage());
            customAssert.assertTrue(false,"Exception "+e.getMessage()+" occurred while executing TC-C153913 and TC-C153906.");
        }finally {
    ...
    @Test
    public void testC153907(){
        CustomAssert customAssert = new CustomAssert();
        int cdrId = -1;
        try{
            cdrId = createCDR(customAssert);
            HashMap<String, String> createdDocument = createDocument(customAssert);
            String uploadDraftResponse = getGeneralUploadDraftResponse(createdDocument,cdrId,customAssert);
            if(ParseJsonResponse.validJsonResponse(uploadDraftResponse)){
                customAssert.assertTrue(new JSONObject(uploadDraftResponse).has("triggerDeviation"),"TC-C153907 has failed.");
            }else{
                logger.error("Draft Request Response is not a valid Response.");
                customAssert.assertTrue(false,"Draft Request Response is not a valid Response.");
            }
        }catch (Exception e){
            logger.error("Exception {} occurred while executing TC-C153907.",e.getMessage());
            customAssert.assertTrue(false,"Exception "+e.getMessage()+" occurred while executing TC-C153907.");
        }finally {
            EntityOperationsHelper.deleteEntityRecord(entityName, cdrId);
            logger.info("CDR with entityId id {} is deleted.", cdrId);
        }
    ...
    public void testC153908(){
        ...
        try{
            ...
            HashMap<String, String> templateDetails = chooseTemplate(customAssert);
            if(addTheTemplateToCDR(cdrId, templateDetails, customAssert)){
                boolean selectTemplateDeviation = false;
                String selectTemplateClusterId = "";
                HashMap<String,HashMap<String,Boolean>> clusterDocDeviation = getClusterAndDeviation(cdrId, customAssert);
                for(String clusterId : clusterDocDeviation.keySet()){
                    for(String docId : clusterDocDeviation.get(clusterId).keySet()){
                        selectTemplateDeviation = clusterDocDeviation.get(clusterId).get(docId);
                        selectTemplateClusterId = clusterId;
                    }
                }

                String thirdPartyDocId = "";
                boolean thirdPartyDeviationFlag = false;
                if(selectTemplateDeviation & uploadTheDocument(createDocument(customAssert),cdrId,false,customAssert)){
                    clusterDocDeviation = getClusterAndDeviation(cdrId, customAssert);
                    for(String clusterId : clusterDocDeviation.keySet()){
                        if(!clusterId.equalsIgnoreCase(selectTemplateClusterId)){
                            for(String docId : clusterDocDeviation.get(clusterId).keySet()){
                                thirdPartyDeviationFlag = clusterDocDeviation.get(clusterId).get(docId);
                                thirdPartyDocId = docId;
                                customAssert.assertFalse(thirdPartyDeviationFlag,"TC-C153907 has failed as 3rd party document has Deviation Tracking on by default.");
                                customAssert.assertFalse(thirdPartyDeviationFlag==selectTemplateDeviation,"TC-C153908 has failed.");
                            }
                        }
                    }
                }else{
                    logger.error("TC-C153906 has failed.");
                    customAssert.assertTrue(false,"TC-C153906 has failed.");
                }

                if(uploadTheDocument(createDocument(customAssert),cdrId,false,customAssert)){
                    clusterDocDeviation = getClusterAndDeviation(cdrId, customAssert);
                    for(String clusterId : clusterDocDeviation.keySet()){
                        if(!clusterId.equalsIgnoreCase(selectTemplateClusterId)){
                            for(String docId : clusterDocDeviation.get(clusterId).keySet()){
                                if(!thirdPartyDocId.equalsIgnoreCase(docId)){
                                    customAssert.assertTrue(thirdPartyDeviationFlag==clusterDocDeviation.get(clusterId).get(docId),"TC-C153908 has failed.");
                                }
                            }
                        }
                    }
                }else{
                    logger.error("Document could not be uploaded twice.");
                    customAssert.assertTrue(false,"Document could not be uploaded twice.");
                }
            }else {
            ...
            logger.error("Exception {} occurred while executing TC-C153908.",e.getMessage());
            customAssert.assertTrue(false,"Exception "+e.getMessage()+" occurred while executing TC-C153908.");
        }finally {
            EntityOperationsHelper.deleteEntityRecord(entityName, cdrId);
            logger.info("CDR with entityId id {} is deleted.", cdrId);
        }
    ...
    @Test
    public void testC153910(){
        CustomAssert customAssert = new CustomAssert();
        int cdrId = -1;
        try{
            cdrId = createCDR(customAssert);
            HashMap<String, String> downloadedTemplate = downloadTemplate(customAssert);
            uploadTheDocument(downloadedTemplate,cdrId,false,customAssert);
            boolean documentDeviation = false;
            String documentId = "";
            HashMap<String,HashMap<String,Boolean>> clusterDocDeviation = getClusterAndDeviation(cdrId, customAssert);
            for(String clusterId : clusterDocDeviation.keySet()){
                for(String docId : clusterDocDeviation.get(clusterId).keySet()){
                    documentDeviation = clusterDocDeviation.get(clusterId).get(docId);
                    documentId = docId;
                }
            }
            boolean deviationOn2ndUpload = false;
            String documentIdOn2ndUpload = "";
            uploadTheDocument(downloadedTemplate,cdrId,true,customAssert);
            clusterDocDeviation = getClusterAndDeviation(cdrId, customAssert);
            for(String clusterId : clusterDocDeviation.keySet()){
                for(String docId : clusterDocDeviation.get(clusterId).keySet()){
                    if(!documentId.equalsIgnoreCase(docId)) {
                        deviationOn2ndUpload = clusterDocDeviation.get(clusterId).get(docId);
                        documentIdOn2ndUpload = docId;
                        customAssert.assertFalse(documentDeviation == deviationOn2ndUpload, "Deviation has not been triggered even though user has opted for the trigger.");
                    }
                }
            }
            ...
            for(String clusterId : clusterDocDeviation.keySet()){
                for(String docId : clusterDocDeviation.get(clusterId).keySet()){
                    if(!documentId.equalsIgnoreCase(docId) & !documentIdOn2ndUpload.equalsIgnoreCase(docId)) {
                        boolean deviationOn3rdUpload = clusterDocDeviation.get(clusterId).get(docId);
                        /*documentIdOn2ndUpload = docId;*/
                        customAssert.assertFalse(documentDeviation == deviationOn3rdUpload, "TC-C153910 has failed.");
                    }
                }
            }
        }catch (Exception e){
            logger.error("Exception {} occurred while executing TC-C153910.",e.getMessage());
            customAssert.assertTrue(false,"Exception "+e.getMessage()+" occurred while executing TC-C153910.");
        }finally {
            EntityOperationsHelper.deleteEntityRecord(entityName, cdrId);
            logger.info("CDR with entityId id {} is deleted.", cdrId);
        }
    ...
    @Test
    public void testC153912(){
        CustomAssert customAssert = new CustomAssert();
        int cdrId = -1;
        try{
            cdrId = createCDR(customAssert);
            HashMap<String, String> createdDocument = createDocument(customAssert);
            if(uploadTheDocument(createdDocument,cdrId,false,customAssert)){
                //Further steps to edit deviation tracking.
            }else{
                logger.error("Document could not be uploaded.");
                customAssert.assertTrue(false,"Document could not be uploaded.");
            }
        }catch (Exception e){
            logger.error("Exception {} occurred while executing TC-C153912.",e.getMessage());
            customAssert.assertTrue(false,"Exception "+e.getMessage()+" occurred while executing TC-C153912.");
        }finally {
            EntityOperationsHelper.deleteEntityRecord(entityName, cdrId);
            logger.info("CDR with entityId id {} is deleted.", cdrId);
        }
...
//Select template
    ...
    private HashMap<String,Boolean> getContractDocumentColumns(int entityId, CustomAssert customAssert){
            ...
            if (ParseJsonResponse.validJsonResponse(contractDocumentsListData)){
                ...
                for(int index=0; index<dataArray.length();index++){
                    for(String key : dataArray.getJSONObject(index).keySet()){
                        try{
                            if(dataArray.getJSONObject(index).getJSONObject(key).getString("columnName").equalsIgnoreCase("triggerdeviation")){
                                triggerDeviation.put("ColumnExist",true);
                                if(dataArray.getJSONObject(index).getJSONObject(key).getString("value").equalsIgnoreCase("yes")){
                                    triggerDeviation.put("DeviationTracking",true);
                                }else{
                                    triggerDeviation.put("DeviationTracking",false);
                                }
                                break;
                            }
                        }catch (Exception e){
    ...
    private boolean uploadTheDocument(HashMap<String, String> downloadedDocument, int entityId, boolean triggerDeviation, CustomAssert customAssert) {
        boolean flag = false;
        try {
            ...
            if (ParseJsonResponse.validJsonResponse(uploadDraftResponse)) {
                ...
                commentDocumentsValue = "{\"templateTypeId\":" + templateTypeId + ",\"documentFileId\":null,\"documentTags\":null,\"triggerDeviation\": "+triggerDeviation+",\"documentSize\":" + documentSize + ",\"key\":\"" + key + "\",\"documentStatusId\":" + documentStatusId + ",\"permissions\":{\"financial\":false,\"legal\":false,\"businessCase\":false},\"performanceData\":false,\"searchable\":false,\"shareWithSupplierFlag\":false}";
                Edit edit = new Edit();
