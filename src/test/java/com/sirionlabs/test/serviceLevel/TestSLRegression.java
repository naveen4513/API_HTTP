...
//				expectedSLMetValue = "Low Volume Rajesh 123";
            ...
            allTestData.add(new Object[]{flowToTest, DCQ, PCQ, expectedSLMetValue});
        }
        return allTestData.toArray(new Object[0][]);
    }
...
//                if(!performWorkFlowActions(slEntityTypeId, serviceLevelId, workFlowSteps, auditLogUser,false, customAssert)) {
//                    customAssert.assertTrue(false, "Error while performing workflow actions on SL ID " + serviceLevelId);
//                    customAssert.assertAll();
//                }

                childServiceLevelIds = checkIfCSLCreatedOnServiceLevel(serviceLevelId, customAssert);
                ...
                if (!validateViewTemplate(Integer.parseInt(childServiceLevelIds.get(0)), null, customAssert)) {
                    customAssert.assertTrue(false, "View Template validated unsuccessfully when no template uploaded on service level");
                }

                if (!validatePerformanceDataFormatTab(serviceLevelId, "", customAssert)) {
...
//					customAssert.assertAll();
                ...
                if (!validateDownloadFileFromPerformanceTab(serviceLevelId, performanceDataFormatFileName + RandomNumbers.getRandomNumberWithinRange(1, 10), performanceDataFormatFileName, customAssert)) {
                    customAssert.assertTrue(false, "Error while validating Download File From Performance Tab for SL ID " + serviceLevelId);
                }
    ...
    @Test(dependsOnMethods = "TestCSLCreation")
    public void testCSLFunc(){

        CustomAssert customAssert = new CustomAssert();

        testCSLFunctionalities(customAssert);

        customAssert.assertAll();
    }

    //    @Test(dependsOnMethods = "TestCSLCreation",enabled = false)
    public void TestEmailForSLTemplateUpload(CustomAssert customAssert){
...
//        customAssert.assertAll();
    ...
    //C10440  C13523
//    @Test(dependsOnMethods = "TestCSLCreation", enabled = false)     //Completed
    public void TestCSLComputationStatusDataNotUploaded(CustomAssert customAssert) {
...
//    @Test(dependsOnMethods = "TestCSLCreation", enabled = false)     //Completed
    public void TestCSLComputationStatusDataMarkedForAggregation(CustomAssert customAssert) {
...
//    @Test(dependsOnMethods = "TestCSLCreation", enabled = false)     //Completed
    public void TestCSLCompStatus_DataMarkedComputation_And_ComputationCompletedSuccessfully(CustomAssert customAssert) {
...
//				Boolean workFlowStatus = workflowActionsHelper.performWorkflowAction(cslEntityTypeId, childServiceLevelId, workFlowActionToPerform);

            if (!workFlowStatus) {
//					customAssert.assertTrue(false, "Unable to perform " + workFlowActionToPerform + " computation on CSL Id " + childServiceLevelId);
...
//        customAssert.assertAll();
    ...
    //    C10562
//    @Test(dependsOnMethods = "TestCSLCreation", enabled = false)         //Revalidated 17 june  //Completed
    public void TestUploadPerformanceDataFormatWrongFileType(CustomAssert customAssert) {
...
//			customAssert.assertAll();
    ...
    public void TestTargetsValueFlowDownFromSLToCSL(String flowToTest) {
        ...
        int serviceLevelIdTargetValues;

        try {
            serviceLevelIdTargetValues = getActiveServiceLevelId(flowToTest, PCQ, DCQ, auditLogUser, customAssert);
            Thread.sleep(5000);
            if (serviceLevelIdTargetValues != -1) {

                ArrayList<String> childServiceLevelId = checkIfCSLCreatedOnServiceLevel(serviceLevelIdTargetValues, customAssert);

                if (childServiceLevelId.size() == 0) {
                    customAssert.assertTrue(false, "Child Service Level not created ");
                    customAssert.assertAll();
                }
                addCSLToDelete(childServiceLevelId);

                int numberOfChildServiceLevel = Integer.parseInt(ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "numberofchildservicelevel"));

                if (childServiceLevelId.size() != numberOfChildServiceLevel) {

                    customAssert.assertTrue(false, "For Service Level Id " + serviceLevelIdTargetValues + " Number of Child Service Level Expected are " + numberOfChildServiceLevel + " Actual " + childServiceLevelId.size());
                    customAssert.assertAll();
                }
                show.hitShowVersion2(slEntityTypeId, serviceLevelIdTargetValues);
                String showResponseSL = show.getShowJsonStr();

                int csl = Integer.parseInt(childServiceLevelId.get(0));
    ...
    @Test(dataProvider = "slMetStatusValidationFlows", enabled = false)
    public void TestSLMetStatus(String flowToTest) {
        ...
        int serviceLevelIdSLMetStatus;

        try {
            serviceLevelIdSLMetStatus = getServiceLevelId(flowToTest, PCQ, DCQ, customAssert);

            slToDelete.add(serviceLevelIdSLMetStatus);

            Thread.sleep(5000);

            if (serviceLevelIdSLMetStatus != -1) {

                if (!validate_Max_SigMax_AbsentSL(serviceLevelIdSLMetStatus, flowToTest, customAssert)) {
                    customAssert.assertTrue(false, "Max Or SigMax Fields are not present or absent on the SL ID " + serviceLevelIdSLMetStatus
                ...
                if (!performWorkFlowActions(slEntityTypeId, serviceLevelIdSLMetStatus, workFlowSteps, auditLogUser,false, customAssert)) {
                    customAssert.assertTrue(false, "Error while performing workflow actions on SL ID " + serviceLevelId);
                    customAssert.assertAll();
                }
                ...
                String performanceDataFormatFileName = ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "performancedataformatfilename");
                String expectedMsg = ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "performancedatuploadsuccessmsg");
                if (!uploadPerformanceDataFormat(serviceLevelIdSLMetStatus, uploadIdSL_PerformanceDataTab, slMetaDataUploadTemplateId, uploadFilePath, performanceDataFormatFileName, expectedMsg, customAssert)) {
                    customAssert.assertTrue(false, "Error while performance data file upload");
                }

                if (!validatePerformanceDataFormatTab(serviceLevelIdSLMetStatus, performanceDataFormatFileName, customAssert)) {

                    customAssert.assertTrue(false, "Error while validating Performance Data Format Tab");
                }

                ArrayList<String> childServiceLevelId = checkIfCSLCreatedOnServiceLevel(serviceLevelIdSLMetStatus, customAssert);
                if (childServiceLevelId.size() == 0) {
                    customAssert.assertTrue(false, "Child Service Level not created ");
                    customAssert.assertAll();
                }
...
//					customAssert.assertAll();
                ...
                int cSLIdForSlMetStatusValidation = Integer.parseInt(childServiceLevelId.get(0));
                String showResponse;

                if (!BulkTemplate.downloadBulkUpdateTemplate(downloadFilePath, bulkUpdateFilename, cslBulkUpdateTemplateId, cslEntityTypeId, childServiceLevelId.get(0))) {
                    customAssert.assertTrue(false, "Unable to download bulk template for CSL Id " + childServiceLevelId.get(0));
//					customAssert.assertAll();
                }
                ...
                for (int i = 0; i < totalStatusToValidate; i++) {
                    ...
                    XLSUtils.updateColumnValue(downloadFilePath, bulkUpdateFilename, "Child Sla", 6, 35, finalNumValues[i]);
...
//					XLSUtils.updateColumnValue(downloadFilePath, bulkUpdateFilename, "Child Sla", 6, 38, slMetStatusToValidate[i]);
                ...
                customAssert.assertTrue(false,"Unable to create Service level for the flow " + flowToTest);
            }
    ...
    //    C10591 C10746
//    @Test(dependsOnMethods = "TestCSLCreation", enabled = false)
    public void TestOperationOnRawDataTab(CustomAssert customAssert) {

//        CustomAssert customAssert = new CustomAssert();
        HashMap<String, String> viewHistoryMap = new HashMap<>();

        int cSlId = Integer.parseInt(childServiceLevelIds.get(5));
        try {
            Thread.sleep(5000);
        }catch (Exception e){
...
//		int numberOfLineItemsOnRawData = 10;
        ...
        if (!validateInclude(cSlId, numberOfLineItemsOnRawData, customAssert)) {
            customAssert.assertTrue(false, "Include Case Validated Successfully for CSL Id " + cSlId);
        }
...
//        customAssert.assertAll();
    ...
    //C44283
    @Test(enabled = false)   //Revalidated 11 june  //Completed   Issue
    public void TestBulkUploadPerformanceData() {
        ...
        try {
            ...
            int serviceLevelId = getActiveServiceLevelId(flowToTest, PCQ, DCQ, auditLogUser, customAssert);
...
//				customAssert.assertAll();
            ...
            ArrayList<String> childServiceLevelIdsBulkUpload = checkIfCSLCreatedOnServiceLevel(serviceLevelId, customAssert);
            addCSLToDelete(childServiceLevelIdsBulkUpload);
            ...
            for (int i = initialCSLToStart; i < totalCSLToUpdate; i++) {
                ...
                entityIds = entityIds + childServiceLevelIdsBulkUpload.get(i) + ",";
            }
...
//            C13601
            for (int i = totalCSLToUpdate - 1; i >= initialCSLToStart; i--) {
                ...
                cslId = Integer.parseInt(childServiceLevelIdsBulkUpload.get(i));
                show.hitShowVersion2(cslEntityTypeId, cslId);
                showResponse = show.getShowJsonStr();
...
//				customAssert.assertAll();
    ...
    //C13773  No Change Required
    @Test(enabled = false,dataProvider = "bulkCreateFlows")
    public void TestBulkCreateSL(String bulkCreateFlow) {
        ...
        try {
            ...
            String bulkCreateSLFile = ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "bulkcreateslfilename");

            int startRowNum = 6;
            int numberOfSlToCreate = 6;

            int bulkCreateExcelRowToUpdateTill = startRowNum + numberOfSlToCreate;

            //Disabling template download as fixing the bulk create template, will need to update the file every time template gets changed
            ...
            String desc = "SLA Creation from Bulk Create" + DateUtils.getCurrentDateInDDMMMYYYY() + RandomNumbers.getRandomNumberWithinRange(1, 100);
            int j =1;

            int supplierDBId = 1050;
            String supplierName = "Berkshire Hathaway - test 1";
            String supplierId = "SP01004";

            for (int i = startRowNum; i < bulkCreateExcelRowToUpdateTill; i++) {
                XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 0, String.valueOf(j));
                j = j+1;
                ...
                XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 47, "80");
                XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 1, supplierName + " ( " + supplierId + " )");
                XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 2, "Sirion Performance Contracts ( CO01035 )");
                XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 13, "Asia/Kolkata (GMT +05:30)");
                XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 16, "United States Dollar (USD)");
                XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 102, "Yes");

                if(bulkCreateFlow.equalsIgnoreCase("sl max level 1")){

                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 46, "Maximum - 1 level");


                }else if(bulkCreateFlow.equalsIgnoreCase("sl max level 2")){
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 46, "Maximum - 2 level");
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 48, expectedMinExpected);

                }else if(bulkCreateFlow.equalsIgnoreCase("sl max level 3")){
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 46, "Maximum - 3 level");
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 48, expectedMinExpected);
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 49, sigMinExpected1);

                }else if(bulkCreateFlow.equalsIgnoreCase("sl min level 1")){
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 46, "Minimum - 1 level");
                }else if(bulkCreateFlow.equalsIgnoreCase("sl min level 2")){
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 46, "Minimum - 2 level");
                }else if(bulkCreateFlow.equalsIgnoreCase("sl min level 3")){

                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 46, "Minimum - 3 level");
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 47, expectedMaxExpected2);
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 48, maxExpected1);
                    XLSUtils.updateColumnValue(uploadFilePath, bulkCreateSLFile, "Sla", i, 49, sigMinExpected2);
                }
            }
...
//				}
    ...
    //C13761 C13760   No Change Required
    @Test(enabled = false)
    public void TestBulkEdit() {
        ...
        try {
            ...
            int serviceLevelId1 = getActiveServiceLevelId(flowToTest, PCQ, DCQ, auditLogUser, customAssert);
//			int serviceLevelId1 = 6745;
            slToDelete.add(serviceLevelId1);

            int serviceLevelId2 = getActiveServiceLevelId(flowToTest, PCQ, DCQ, auditLogUser, customAssert);
//			int serviceLevelId2 = 6746;
            slToDelete.add(serviceLevelId2);

            ArrayList<String> childServiceLevelIsList1 = checkIfCSLCreatedOnServiceLevel(serviceLevelId1, customAssert);

            if (childServiceLevelIsList1.size() == 0) {
                customAssert.assertTrue(false, "Child Service Level not created ");
                customAssert.assertAll();
            }

            addCSLToDelete(childServiceLevelIsList1);
            int cslID1 = Integer.parseInt(childServiceLevelIsList1.get(0));

            ArrayList<String> childServiceLevelIsList2 = checkIfCSLCreatedOnServiceLevel(serviceLevelId2, customAssert);

            if (childServiceLevelIsList2.size() == 0) {
                customAssert.assertTrue(false, "Child Service Level not created ");
                customAssert.assertAll();
            }

            int cslID2 = Integer.parseInt(childServiceLevelIsList2.get(0));
            addCSLToDelete(childServiceLevelIsList2);
...
//					"\"actualPerformance\": {\"name\": \"actualPerformance\",\"id\": 1102,\"values\": \"" + actualPerformance + "\"}," +
            ...
            if (!verifyAuditLog(cslEntityTypeId, cslID1, "Updated (Bulk)", auditLogUser, customAssert)) {
                customAssert.assertTrue(false, "Audit Log Tab Verified successfully after Bulk Edit");
            }

            if (!verifyAuditLog(cslEntityTypeId, cslID2, "Updated (Bulk)", auditLogUser, customAssert)) {
                customAssert.assertTrue(false, "Audit Log Tab Verified successfully after Bulk Edit");
            }
...
//			showPageFieldValuesMap.put("actualperformance", actualPerformance);
    ...
    @Test(dataProvider = "slMetCalculationThroughES", enabled = false)
    public void TestSLMETStatusCalculationESQuery(String flow, String DCQ, String PCQ, String expectedSLMetValue) {
...
//		cslIdForSLMetCalcESQuery = 27069;
//		slIdForSLMetCalcESQuery = 6840;
        try {
            slIdForSLMetCalcESQuery = getActiveServiceLevelId(flowToTest, PCQ, DCQ,auditLogUser,false, customAssert);
            slToDelete.add(slIdForSLMetCalcESQuery);
//
            if (slIdForSLMetCalcESQuery != -1) {
//
////
                ArrayList<String> childServiceLevelIds = checkIfCSLCreatedOnServiceLevel(slIdForSLMetCalcESQuery, customAssert);
                ...
                cslIdForSLMetCalcESQuery = Integer.parseInt(childServiceLevelIds.get(1));
...
//					customAssert.assertAll();
                ...
                if (!uploadRawDataCSL(childServiceLevelId, rawDataFileName, rawDataFileValidMsg, customAssert)) {

                    customAssert.assertTrue(false, "Raw Data Uploaded unsuccessfully on CSL " + childServiceLevelId);
//					customAssert.assertAll();
                } else {

                    if (!validateStructuredPerformanceDataCSL(childServiceLevelId, rawDataFileName, "Done", "View Structured Data", auditLogUser, customAssert)) {
                        customAssert.assertTrue(false, "Raw Data Upload validation unsuccessful for CSL ID " + childServiceLevelId);
                        customAssert.assertAll();
                    }
                }

                Boolean workFlowStatus = performComputationCSL(childServiceLevelId, customAssert);
//				String workFlowActionToPerform = "ReComputePerformance";
//				WorkflowActionsHelper workflowActionsHelper = new WorkflowActionsHelper();
//				Boolean workFlowStatus = workflowActionsHelper.performWorkflowAction(cslEntityTypeId, childServiceLevelId, workFlowActionToPerform);
                ...
                if (!workFlowStatus) {
//					customAssert.assertTrue(false, "Unable to perform " + workFlowActionToPerform + " computation on CSL Id " + childServiceLevelId);
                    customAssert.assertTrue(false, "Unable to perform  computation on CSL Id " + childServiceLevelId);
                } else {
...
//					customAssert.assertAll();
                ...
                if (updatePCQonMSL(serviceLevelId, PCQUpdated, customAssert)) {

                    if (!checkDCQPCQUDCOnCSL(childServiceLevelId, DCQ, PCQUpdated, "Incident ID", customAssert)) {
                        customAssert.assertTrue(false, "DCQ PCQ validated unsuccessfully on CSL ID " + childServiceLevelId);
                    }

                } else {
                    customAssert.assertTrue(false, "PCQ updated unsuccessfully on SL " + serviceLevelId);
                }
    ...
    @Test(dataProvider = "timeDifferenceFunctionCalculationDifferentFlows",enabled = false)
    public void TestTimeDifferenceFunction(String dateFormat){
        ...
        try{
            ...
            serviceLevelId = getActiveServiceLevelId(flowToTest, PCQ, DCQ,auditLogUser,false, customAssert);
            Thread.sleep(5000);
            slToDelete.add(serviceLevelId);
//			serviceLevelId = 9139;
            if (serviceLevelId != -1) {
...
//					customAssert.assertAll();
                ...
                int childServiceLevelId = Integer.parseInt(childServiceLevelIds.get(0));

//				int childServiceLevelId = 34806;
                ...
                if (dateFormat.equalsIgnoreCase("MM/dd/yyyy") || dateFormat.equalsIgnoreCase("dd/MM/yyyy")) {
                    ...
                    for (int i = 1; i < 8; i++) {
                        ...
                        startDate = XLSUtils.getOneCellValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 2);
                        startDate = startDate.replaceAll("-", "/");
                        XLSUtils.updateColumnValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 2, startDate);


                        endDate = XLSUtils.getOneCellValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 2);
                        endDate = endDate.replaceAll("-", "/");
                        XLSUtils.updateColumnValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 3, endDate);

                    }
                }

                if (!uploadRawDataCSL(childServiceLevelId, rawDataFileNameToUpload, rawDataFileValidMsg, customAssert)) {

                    customAssert.assertTrue(false, "Raw Data Uploaded unsuccessfully on CSL " + childServiceLevelId);
//					customAssert.assertAll();
                } else {

                    if (!validateStructuredPerformanceDataCSL(childServiceLevelId, rawDataFileNameToUpload, "Done", "View Structured Data", auditLogUser, customAssert)) {
                        customAssert.assertTrue(false, "Raw Data Upload validation unsuccessful for CSL ID " + childServiceLevelId);
                        customAssert.assertAll();
                    }
                }
                ...
                try{
                    ...
                    for(int i =1 ;i<8;i++) {
                        ...
                        try {
                            ...
                            endDate = XLSUtils.getOneCellValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 3);


                            weekTypeStr = XLSUtils.getOneCellValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 5);
                            workingHours = XLSUtils.getOneCellValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 6);
                            timeZone = XLSUtils.getOneCellValue(uploadFilePath, rawDataFileNameToUpload, "Raw Data", i, 7);
                ...
                Boolean timeDifferenceValidationStatus =  validateTimeDifferenceValues(childServiceLevelId,timeDifferenceExpectedList,customAssert);
    ...
    //Disabling the test case when ba pod env is ready will run
    @Test(enabled = false)
    public void TestSingleFormatBulkUpload(){
        ...
        try{
            ...
            int serviceLevelId1;
            int serviceLevelId2;
            ArrayList<String> childServiceLevelIds1;
            ArrayList<String> childServiceLevelIds2;
            ...
            serviceLevelId1 = getActiveServiceLevelId(flowToTest, PCQ, DCQ,auditLogUser,false, customAssert);
            serviceLevelId2 = getActiveServiceLevelId(flowToTest, PCQ, DCQ,auditLogUser,false, customAssert);

            slToDelete.add(serviceLevelId1);
            slToDelete.add(serviceLevelId2);

            childServiceLevelIds1 = checkIfCSLCreatedOnServiceLevel(serviceLevelId1,customAssert);
            childServiceLevelIds2 = checkIfCSLCreatedOnServiceLevel(serviceLevelId2,customAssert);
            Thread.sleep(5000);
            ...
            if (!uploadPerformanceDataFormat(serviceLevelId2, uploadIdSL_PerformanceDataTab, slMetaDataUploadTemplateId, fileUploadPath, performanceDataFormatFileName, expectedMsg, customAssert)) {
                customAssert.assertTrue(false, "Error while performance data file upload");
            }

            int numberOfCslCreated = childServiceLevelIds1.size();
...
//			String masterFileName = "CSL Raw Data BA POD";
            ...
            for(int i =0;i<cslSizeToUploadRawData;i++){
                ...
                try{
                    childServiceLevelIds1.get(i);
                    childServiceLevelIds2.get(i);

                    textBodyMap.put("cslIds[" + j +"]",childServiceLevelIds1.get(i));
                    j =j +1;
                    textBodyMap.put("cslIds[" + j +"]",childServiceLevelIds2.get(i));
                    j =j +1;
            ...
            for(int i =0;i<cslSizeToUploadRawData;i++) {
                cslId = Integer.parseInt(childServiceLevelIds1.get(i));
            ...
            for(int i =0;i<cslSizeToUploadRawData;i++) {
                cslId = Integer.parseInt(childServiceLevelIds2.get(i));
    ...
    //    @Test(dependsOnMethods = "TestCSLCreation",enabled = false)
    public void TestDownloadRawDataFormat(CustomAssert customAssert){

//        CustomAssert customAssert = new CustomAssert();

        try{
//			int serviceLevelId = 6798;
            Map<String,String> urlStringMap = new HashMap<>();
//
            urlStringMap.put("entityId",String.valueOf(serviceLevelId));
            ...
            if(!fileDownloadStatus){
                customAssert.assertTrue(false,"File Download done unsuccessfully for SL ID " + serviceLevelId);
            }
...
//                + "order by id desc";
    ...
    private int getActiveServiceLevelId(String flowToTest, String PCQ, String DCQ, String user,Boolean checkAuditLogTab, CustomAssert customAssert) {
        ...
        try {
            ...
            if (serviceLevelId != -1) {
                List<String> workFlowSteps;
                workFlowSteps = Arrays.asList(ParseConfigFile.getValueFromConfigFile(configFilePath, configFileName, "slactiveworkflowsteps").split("->"));
                if (!performWorkFlowActions(slEntityTypeId, serviceLevelId, workFlowSteps, user,checkAuditLogTab, customAssert)) {
                    customAssert.assertTrue(false, "Error while performing workflow actions on SL ID " + serviceLevelId);
                    customAssert.assertAll();
                    return -1;
                }
    ...
    private int approveCSL(int childServiceLevelId,CustomAssert customAssert) {

        String actionName = "ApproveComputation";
        WorkflowActionsHelper workflowActionsHelper = new WorkflowActionsHelper();
        try {
//
//			if (!workflowActionsHelper.performWorkflowAction(cslEntityTypeId, childServiceLevelId, actionName)) {
            if (!workflowActionsHelper.performWorkFlowStepV2(cslEntityTypeId, childServiceLevelId, actionName,customAssert)) {
                childServiceLevelId = -1;
            }
        } catch (Exception e) {
            logger.error("Exception while performing " + actionName + "on CSL ID " + childServiceLevelId + " " + e.getMessage() );
            childServiceLevelId = -1;
        }

        return childServiceLevelId;
    }

    private Boolean updateDCQPCQUDCOnSL(int serviceLevelId, String PCQUpdated, String DCQUpdated, String UDCUpdated, CustomAssert customAssert) {

        logger.info("Updating PCQ and DCQ on Service Level ID " + serviceLevelId);
        ...
        try {
            ...
            if (APIUtils.validJsonResponse(editResponse)) {
                ...
                if (!editResponse.contains("\"status\":\"success\"")) {
                    customAssert.assertTrue(false, "Service Level ID " + serviceLevelId + " updated unsuccessfully");
                    updateStatus = false;
                }

            } else {
                customAssert.assertTrue(false, "Edit Response is not a valid Json for service Level " + serviceLevelId);
                updateStatus = false;
            }
        } catch (Exception e) {
            customAssert.assertTrue(false, "Exception while editing service level ID " + serviceLevelId + " for DCQ and PCQ " + e.getMessage());
            updateStatus = false;
        }
...
//			}
    ...
    private Boolean checkIfGraphDataGenerated(int listId,int serviceLevelIdForGraphValidation,int calendarType,CustomAssert customAssert){
        ...
        try{
            ...
            while (timeOut < waitTimeOut) {
                try {
                    listRendererListData.hitListRendererSlaSpecificGraph(listId, serviceLevelIdForGraphValidation, calendarType);
    ...
    public void testCSLFunctionalities(CustomAssert customAssert)
    {
        ...
        Callable<Void> callable1 = new Callable<Void>()
        {
            ...
            {
                TestOperationOnRawDataTab(customAssert);
                return null;
            }
        };
        ...
        Callable<Void> callable2 = new Callable<Void>()
        {
            ...
            public Void call() throws Exception
            {
                TestCSLComputationStatusDataNotUploaded(customAssert);
                return null;
            }
        };
        ...
        Callable<Void> callable3 = new Callable<Void>()
        {
            ...
            public Void call() throws Exception
            {
                TestCSLComputationStatusDataMarkedForAggregation(customAssert);
                return null;
            }
        };
        ...
        Callable<Void> callable4 = new Callable<Void>()
        {
            ...
            public Void call() throws Exception
            {
                TestCSLCompStatus_DataMarkedComputation_And_ComputationCompletedSuccessfully(customAssert);
                return null;
            }
        };
        ...
        Callable<Void> callable5 = new Callable<Void>()
        {
            ...
            public Void call() throws Exception
            {
                TestUploadPerformanceDataFormatWrongFileType(customAssert);
                return null;
            }
        };
        ...
        Callable<Void> callable6 = new Callable<Void>()
        {
            ...
            public Void call() throws Exception
            {
                TestDownloadRawDataFormat(customAssert);
                return null;
            }
        };
